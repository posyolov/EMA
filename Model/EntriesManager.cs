...
namespace Model
{
    public class EntriesManager : IEntityManager<Entry>
    {
        ...
        {
            return entriesRepository.GetWithInclude(par => par.Parent, pos => pos.Position, ps => ps.Parent.Position, r => r.Reason, c => c.ContinuationCriteria, u => u.ChangeUser, eu => eu.AssignedUsers).OrderBy(d => d.OccurDateTime);
        }
        ...
        public IEnumerable<Entry> GetEntriesTree()
        {
            ...
            return entriesRepository.GetWithInclude(false, d => d.OccurDateTime, pos => pos.Position, r => r.Reason, c => c.ContinuationCriteria, u => u.ChangeUser, eu => eu.AssignedUsers).Where(p => p.ParentId == null);
        }
